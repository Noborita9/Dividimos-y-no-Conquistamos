...
struct ST {
    ...
    void update(int v, int L, int R, int ql, int qr, ll w){
        ...
        push(v, L, R);
        if (ql == L && qr == R){
            st[v].mx += w; // Update acutal node
            lz[v] += w; // Add lazy
            push(v, L, R); // Initial spread
            return;
        }
        int m = MD(L, R);
